...
import { Container, Grid, Card, Typography, Button, Box, CardContent, Avatar, Rating, ListItem } from '@mui/material';
  ...
  .react-calendar__month-view__days__day--weekend {
    ...
    .react-calendar__tile--range {
      ...
      background-color:rgba(74, 81, 238, 0.8);
       ...
       .react-calendar__tile.start-date {  /* Custom style for start-date */
    background-color: rgba(238, 159, 74, 0.8)
...
const TourDetail = ({ tour, otherTours }) => {
  ...
  useEffect(() => {
    ...
    const generatedFakeReviews = Array.from({ length: 10 }, (_, index) => ({
      ...
      rating: (index % 5) + 1,
      review: `This is review number ${index + 1}. The experience was amazing, and I would recommend this tour!`,
    }));
  ...
  const tileDisabled = ({ date, view }) => {
    ...
    switch (view) {
      ...
      case 'century': {
        ...
        const startYears = startDates.map(startDate => startDate.getFullYear());
        const startDecades = startYears.map(year => Math.floor(year / 10) * 10);
  ...
  const [selectedImage, setSelectedImage] = useState(tour.images !== null ? tour.images[0] : '');
    ...
    <Container>
      ...
      <section style={{ marginTop: '80px' }}>
        ...
        <Box display="flex" justifyContent="space-between" mt={2}>
          ...
          <Box flex={1} display="flex" justifyContent="center" sx={{ backgroundColor: 'gray', paddingBottom: '20px', margin: '20px', border: '1px solid grey' }}>
            <CalendarContainer>
              <Calendar sx={{ width: '500px', height: '200px' }}
                onChange={handleDateChange}
                value={selectedDate}
                tileDisabled={tileDisabled}
                tileClassName={({ date, view }) => (view === 'month' && isStartDate(date) ? 'start-date' : null)}
              />
            </CalendarContainer>
          </Box>
      ...
      <SimpleTreeView>
        {address && address.map((address, index) => (
          <TreeItem
            ...
            label={<span style={{ fontSize: "2.2rem" }}>Day {index + 1}: {address}</span>}
            key={index}
          ...
          >
            ...
            {schedule && schedule[index].map((schedule, idx) => (
              <TreeItem
                itemId={`${index}-${idx}`}
                label={<span style={{ fontSize: "1.8rem" }}>{schedule}</span>}
                key={`${index}-${idx}`}
      ...
      <section style={{ marginTop: '80px' }}>
        ...
        <Grid container spacing={4} padding={3}>
          {otherTours.slice(0, 3).map((otherTour) => (
            <Grid item xs={12} sm={6} md={4} key={otherTour.id} style={{ position: 'relative' }}>
              <Card
                style={{
              ...
              >
                ...
                >
                  <Typography variant="h4" gutterBottom>{otherTour.name}</Typography>
                </Box>
                ...
                >
                  <Typography variant="h4" style={{ marginLeft: '45px', marginBottom: '5px', marginRight: '8px', color: 'red' }}>
      ...
      <section style={{ marginTop: '80px' }}>
        ...
        <Box sx={{
          ...
          {fakeReviews && fakeReviews.length > 0 ? (
            fakeReviews.slice(0, showMoreReviews ? fakeReviews.length : 3).map((review, index) => (
              <Card key={index} sx={{
                minWidth: '300px',
                height: '400px',
                maxWidth: '358px',
            ...
            <Typography variant="h4" sx={{ color: 'gray' }}>No reviews yet.</Typography>
          )}
        </Box>
        ...
        {fakeReviews.length > 3 && (
          <Box textAlign="center" mt={2}>
            <Button onClick={handleToggleShowMore} variant="outlined" color="primary">

 ...
 const allDates= eachDayOfInterval({
  ...
  const data = allDates.map(date=>{
    return {
      ...
      totalSales: fakeData.filter(booking=>isSameDay(date,new Date(booking.label))).reduce((acc,cur)=>acc+cur.totalPrice,0),
      extrasSales: fakeData.filter(booking=>isSameDay(date,new Date(booking.label))).reduce((acc,cur)=>acc+cur.extrasPrice,0)
    }
  })
  ...
  return (
    <StyledSalesChart>
      ...
      <Tooltip contentStyle={{ backgroundColor:colors.background }}/>
        ...
        strokeWidth={2} name="Extras sale"  unit="$"

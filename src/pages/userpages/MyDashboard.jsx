...
export default function MyDashboard() {
    ...
    return (
        ...
        >
            ...
            <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', maxWidth: '90vw' }}>
                    ...
                    <Grid item xs={12} md={6}>
                        <Box sx={{ background: 'rgba(73, 101, 183, 0.9)', borderRadius: '8px', p: 2 }}>
                            ...
                            {upcomingTourList.length > 0 ? (
                                ...
                                <Typography variant="h5">No upcoming booking available.</Typography>
                            )}
                        </Box>
                    </Grid>
...
const InfoCard = ({ count, label, color, icon, onClick }) => (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        <Box sx={{ width: '100px', height: '100px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'absolute', zIndex: 1 }}>
            <Typography variant="h2" sx={{ color, paddingBottom: '30px' }}><strong>{count}</strong></Typography>
        </Box>
        <Avatar sx={{ width: 130, height: 130, backgroundColor: 'transparent', border: '2px solid black' }}>
            {icon}
        </Avatar>
        <Typography variant="h4" sx={{ color: 'black', mt: 1, cursor: onClick ? 'pointer' : 'default', '&:hover': { color: onClick ? color : 'inherit' } }} onClick={onClick}>{label}</Typography>
    </Box>
);
...
const ImageSlider = ({ upcomingBookings }) => {
    ...
    return (
        <section>
            <BookingDetailModal open={open} handleClose={handleClose} booking={selectedBooking} />
            <Container>
                <Box sx={{ mt: 5 }}>
                    <Slider {...settings}>
                        {upcomingBookings.map((booking, index) => (
                            ...
                            >
                                ...
                                />
                                <Typography variant="h4" sx={{ color: 'var(--color-grey-50)', marginBottom: '5px' }}>
                                    {booking.tour.name}
                                </Typography>
                                <Typography variant="h6" sx={{ color: 'var(--color-grey-100)' }}>
